...
const LOCALES: Record<string, Messages> = { en, it, es, pt, fr };

// endonyms for the explicit selector
export const LANGS: Record<keyof typeof LOCALES, string> = {
  en: 'English', it: 'Italiano', es: 'Español', pt: 'Português', fr: 'Français',
};

// dictionaries are bound at load, so switching = persist + reload
// (instant with the PWA cache; keeps every string non-reactive)
export function setLocale(l: string): void {
  try { localStorage.setItem('stretchTimer.lang', l); } catch { /* private mode */ }
  location.reload();
}
